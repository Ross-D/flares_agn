...
for i in range(9):
    ...
    plt.plot(nu, transmitted, c = cmap(norm(AGN_L[i-1])), label = "1e"+str(AGN_L[i-1])+r" $erg s^{-1}$")
plt.yscale('log')
plt.xscale('log')
plt.ylim(1e33, 1e49)
plt.xlim(1e-2, 1e7)
...
#fig.colorbar.set_label('Temperature /K', rotation=270)
#plt.legend()
fig.savefig("Plots/Luminosity/lambda_transmitted.png")
fig.clf()
